...
export default function NotesListScreen() {
  ...
  const handleUploadPhoto = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      const newFile = { id: generateId(), title: result.assets[0].fileName || 'Uploaded Photo', content: '', checklist: [], colorId: 'default' as any, folderId: currentFolderId, createdAt: Date.now(), updatedAt: Date.now(), isPinned: false, type: 'image' as const, fileUri: result.assets[0].uri };
      await saveNote(newFile);
    }
  };
  ...
  const handleUploadVideo = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      const newFile = { id: generateId(), title: result.assets[0].fileName || 'Uploaded Video', content: '', checklist: [], colorId: 'default' as any, folderId: currentFolderId, createdAt: Date.now(), updatedAt: Date.now(), isPinned: false, type: 'video' as const, fileUri: result.assets[0].uri };
      await saveNote(newFile);
    }
  };
  ...
  const handleUploadPDF = async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets.length > 0) {
        const newFile = { id: generateId(), title: result.assets[0].name || 'Uploaded PDF', content: '', checklist: [], colorId: 'default' as any, folderId: currentFolderId, createdAt: Date.now(), updatedAt: Date.now(), isPinned: false, type: 'pdf' as const, fileUri: result.assets[0].uri };
        await saveNote(newFile);
      }
            ...
            item.type === 'folder' ? (
              <Pressable
                ...
                style={[
                  ...
                  draggingItem?.overFolderId === item.folder.id && [styles.explorerRowDropTarget, { backgroundColor: GlassTheme.accentPrimary + '2E', borderColor: GlassTheme.accentPrimary + 'B3' }],
                ]}
                onLayout={(event) => {
        ...
        >
          ...
          >
            ...
            <View style={styles.modalActions}>
              ...
              <Pressable onPress={handleSaveFolder} style={[styles.modalSave, { backgroundColor: GlassTheme.accentPrimary }]}>
      ...
      <Modal visible={showUploadSheet} transparent animationType="fade">
        <Pressable style={[styles.modalOverlay, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 12 }]} onPress={() => setShowUploadSheet(false)}>
          <View style={[styles.actionSheet, { maxHeight: '82%' }]} onStartShouldSetResponder={() => true}>
            <Text style={styles.actionSheetTitle}>Upload File</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              <Pressable style={styles.actionSheetButton} onPress={handleUploadPhoto}>
                <MaterialIcons name="image" size={16} color={GlassTheme.textSecondary} />
                <Text style={styles.actionSheetText}>Upload Photo</Text>
              </Pressable>
              <Pressable style={styles.actionSheetButton} onPress={handleUploadVideo}>
                <MaterialIcons name="movie" size={16} color={GlassTheme.textSecondary} />
                <Text style={styles.actionSheetText}>Upload Video</Text>
              </Pressable>
              <Pressable style={styles.actionSheetButton} onPress={handleUploadPDF}>
                <MaterialIcons name="picture-as-pdf" size={16} color={GlassTheme.textSecondary} />
                <Text style={styles.actionSheetText}>Upload PDF</Text>
              </Pressable>
              <Pressable style={[styles.actionSheetButton, { marginTop: 8 }]} onPress={() => setShowUploadSheet(false)}>
                <MaterialIcons name="close" size={16} color={GlassTheme.textSecondary} />
